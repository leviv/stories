<script lang="ts">
  ...
  const borderImageSource = isSender 
    ? rightBubbleSvg 
    : isTyping ? typingIndicatorSvg : leftBubbleSvg;
...
>
  {#if showReactionMenu}
    <div class="reaction-overlay" on:pointerdown|stopPropagation={() => showReactionMenu = false}></div>
  {/if}
  ...
  >
    {#if showReactionMenu}
      <div class="reaction-menu {isSender ? 'sender' : 'receiver'}" in:scale={{duration: 150, start: 0.8}}>
    ...
    {#if reactionType}
      <div class="reaction-badge {isSender ? 'sender' : 'receiver'}" in:scale={{duration: 200}}>
        <img src="/messages/reactions/{isSender ? 'left' : 'right'}-light-{reactionType}.svg" alt="reaction" />
      </div>
    {/if}
    {#if !hideTail}
...
<style>
  ...
  .border-artifact-cover.sender {
    top: -17px; right: -22px; bottom: -17px; left: -17px;
  }
  .border-artifact-cover.receiver {
    top: -17px; right: -17px; bottom: -17px; left: -22px;
  }
