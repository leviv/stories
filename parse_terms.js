...
const enText = fullText.substring(fullText.indexOf('IMPORTANT NOTICE TO USER:'), fullText.indexOf('用户服务协议')).trim();
...
const parseEn = (text) => {
    ...
    for (let i = 1; i < parts.length; i += 2) {
        ...
        let paragraphs = bodyRaw.split(pRegex).map(p => p.replace(/\n/g, ' ').replace(/\s+/g, ' ').trim()).filter(Boolean);
